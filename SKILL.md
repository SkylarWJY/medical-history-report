...
- **`lab_history` 健康追踪（也适合没生病的人）**：把多次体检的关键指标（血脂/血糖/肝肾/甲功/肿瘤标志物等）
  连成一张趋势表，`flags` 标出每次的 high/low（自动标红），`verdict` 给"一直异常 / 在变差 / 已正常"。
- **`recheck_calendar` 该复查日历**：每个部位下次该做的检查 + 时间，到点提醒家属去做。
